...
Open the project that you built last time, linking to your group repository, and open up your `Replication_<YOURNAME>.qmd` file. You'll be taking notes (documenting what you are doing and why), and writing/testing code in that notebook over the next several weeks. As you work through this notebook, add your work to `Replication_<YOURNAME>.qmd` -- you can *render* and `Pull -> Commit -> Push` as often as you like. Doing so will be helpful to your you and to your groupmates.
...
```{r}
#Use this code chuck to collapse your genome into a single string using paste(randGenome, collapse="")
```
...
**Challenge 6:** Generate a random genome substring consisting of 10 nucleotides and use the `paste()` method to collapse your genome to a single string rather than a list. Then write a `for` loop to print out each individual nucleotide.
...
Let's see an example of how `str_sub()` works by generating our small genome of 10 nucleotides and extracting the nucleotide in the 7th position. Run the code below and analyze the output.
...
print(str_sub(randGenome,start=7, end=7 ))
```

Now, let's create a for loop that will go through our random genome of 10 nucleotides and print each individual nucleotide.

Remember, to do this you will have to:

-    Set an iterator (let's say `i`).

-   Assign a range for the iterator (What would this be for our genome of 10 nucleotides?).

-    Tell the for loop to use the functions `print(str_sub(string="",start="", end=""))`. What will you set the `start` and `end` arguments to? Remember, we want the for loop to go through each nucleotide in our genome.

When you think you've got it, test your code in the code chuck below.
...
*Hint:* You'll want to initialize a counter variable to 0 before the start of your loop and increment it by one every time the nucleotide is Adenine, rather than simply printing out the nucleotide. This is similar to what we did with our `mySum` loop, but we want to add "1" to our counter for each Adenine.
...
*Hint:* You'll need a separate counter and `if` statement for each of the nucleotides.
...
for(i in 1:nchar(randGenome)){
  ...
  if(str_sub(randGenome, start = i, end = i) == "T"){
    Thyamine_count <- Thyamine_count +1
  }
}
    
print(c(Adenine_count, Thyamine_count, Guanine_count, Cytosine_count))
```
...
```{r}
vib_c <- scan("~/Desktop/longgenome.txt", what = "character", sep = NULL)
```
...
for(i in 1:nchar(vib_c)){
  ...
  if(str_sub(vib_c, start = i, end = i) == "T"){
    Thyamine_count2 <- Thyamine_count2 +1
  }
}

print(c(Adenine_count2, Thyamine_count2, Guanine_count2, Cytosine_count2))
```
...
```{r}
rosalind_c <- scan("~/Desktop/rosalind_dna.txt", what = "character", sep = NULL)
...
Thyamine_count3 <-0
for(i in 1:nchar(vib_c)){
  if(str_sub(rosalind_c, start = i, end = i) == "A"){
    Adenine_count3 <- Adenine_count3 +1
  }
  if(str_sub(rosalind_c, start = i, end = i) == "C"){
    Cytosine_count3 <- Cytosine_count3 +1
  }
  if(str_sub(rosalind_c, start = i, end = i) == "G"){
    Guanine_count3 <- Guanine_count3 +1
  }
  if(str_sub(rosalind_c, start = i, end = i) == "T"){
    Thyamine_count3 <- Thyamine_count3 +1
  }
}

print(c(Adenine_count3, Thyamine_count3, Guanine_count3, Cytosine_count3))
```
